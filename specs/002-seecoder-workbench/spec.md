...
| FR-208 | 文件与附件 | 文本和 PNG/JPEG 可作为上下文附件，超限被拒绝；项目文件由 Agent 工具按需读取 |
...
| FR-220 | 验证新鲜度 | 修改后没有当前 revision 的成功验证时允许完成，并在内部结果中记录验证状态；普通界面不显示独立警告卡片 |
...
- Pull Request 页面通过本机 `gh` 读取当前仓库的开放 PR。IPC 只返回结构化的“可用、未安装、未登录、读取失败”状态，界面不得展示命令返回对象、原始 stderr 或实现说明；未配置时只给出简短、可执行的安装或登录指引。
