...
真实长任务表明，仅有硬上限还不够：若第 24 轮刚完成验证，模型没有下一轮调用 `finish` 的机会，正确工作也会落入 `limitReached`。因此第 22 轮起必须注入收敛提示，要求停止扩展范围，只完成必要验证并调用 `finish`。硬上限仍保持 24，不用无限“宽限轮次”掩盖失控循环。

## 12.1 失败观察与补丁协议

- `run_command` 无论成功、失败、超时或取消，结构化 ToolResult 都保留已捕获的 `exitCode/stdout/stderr`。流式 `tool.output` 服务 UI，ToolResult 服务模型恢复；两者职责不同，不能因为 `ok=false` 丢弃 Observation。
- `apply_patch` 同时接受标准 unified diff 和模型常见的 `*** Begin Patch` / `*** Update File` 格式。两种格式最终进入同一套“先验证全部旧内容，再原子写入”流程。
- Auto 模式对补丁中可解析且全部位于工作区内的路径自动批准；无法解析路径时退回审批，不能因为 `apply_patch` 参数没有单独的 `path` 字段而永远要求确认。

## 12.2 事件追加顺序

同一 Thread 的事件必须按调用顺序串行追加到 JSONL。命令 stdout/stderr 回调与 `tool.completed` 可能在相邻事件循环中并发到达；存储层使用每 Thread 独立写入队列，在队列任务内分配 `seq` 并追加文件。这样既不阻塞其他 Thread，也保证磁盘物理顺序、`seq`、恢复顺序和实时 UI 顺序一致。不能只在读取时排序，因为进程可能在并发写入之间退出，且尾行恢复无法修复已经颠倒的因果关系。
