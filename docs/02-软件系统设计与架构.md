...
| Renderer | 交互、任务列表、审批、Diff 和终端 | 可信业务状态 |
...
浏览器加本地 HTTP 服务会引入端口、鉴权和服务生命周期；纯 CLI 难以展示审批、Diff 和实时工具状态；微服务不适合单用户本地应用。Electron 同时提供本机后端和成熟 UI 生态，代价是安装体积与内存较大。
...
Renderer 发生展示错误不能改变 Core 状态。即使对话工具卡或右侧检查器渲染失败，后台 Turn 仍按 Core 规则运行；全局前端错误处理应提示用户，而不是发送额外副作用。
